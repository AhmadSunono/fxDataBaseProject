...
public class ImportBillController implements Initializable {
    ...
    void buttonHandler(ActionEvent event) throws SQLException {
        if(event.getSource().toString().contains("saveButton")) {
            ...
            // هون بنشوف اول اشي في اسم موزع ولا لأ، اذا لا خلص بنضيف عالستور مباشرة بدون بوت ايتم
            // اذا اه بنعمل الكويري هاي الي تحت
            // بعدين بنحدث الستور تحديث ،اذا الايتمز موجودات بنزيد عليهم واذا لا بنضيفهم         
   ...
   ///we should make it here
            	...
            	ResultSet rs=statement.executeQuery(q);	
            		...
            		q="insert into stored values ("+data.get(i).getSellPrice()+",'"+data.get(i).getBarcode()+"','"+data.get(i).getName()+"',"+data.get(i).getBuyPrice()+","+data.get(i).getQuant()+",TO_DATE('"+data.get(i).getExpDate()+"','YYYY-MM-DD'))";	
            		statement.executeUpdate(q);	
            		}
            		
            		else {
            		q="update stored set quantity=quantity+"+data.get(i).getQuant()+"where barcode ='"+data.get(i).getBarcode()+"'";	
            		statement.executeUpdate(q);
            		q="update stored set exp_date=TO_DATE('" + data.get(i).getExpDate()+"','YYYY-MM-DD') where barcode = '"+data.get(i).getBarcode()+"'";
            		statement.executeUpdate(q);
                   ...
                   ResultSet  rs=statement.executeQuery(q);	
                     	...
                     	String bar="";
                     	if(count3>=0&&count3<10)bar="00000000"+count3;
                     	else if (count3>=10&&count3<=100)bar="0000000"+count3;
                     	else bar="000000"+count3;
                ...
                q="select * from stored where name ='"+data.get(i).getName()+"'";
                    ...
                    if(count!=0) {
                    	q="update stored set quantity=quantity+"+data.get(i).getQuant()+" where name='"+data.get(i).getName()+"'";
                    	statement.executeUpdate(q);
                    	q="update stored set exp_date=TO_DATE('" + data.get(i).getExpDate()+"','YYYY-MM-DD') where name = '"+data.get(i).getName()+"'";
                		statement.executeUpdate(q);
                    }
                    else {
                    	q="insert into stored values ("+data.get(i).getSellPrice()+",'"+bar+"','"+data.get(i).getName()+"',"+data.get(i).getBuyPrice()+","+data.get(i).getQuant()+",TO_DATE('"+data.get(i).getExpDate()+"','YYYY-MM-DD'))";	
                		statement.executeUpdate(q);	
            ...
            for(int i=0;i<data.size();i++) {
            	...
            	while(rs.next()) {count++;}
            	if(count==0) {
            	q="insert into bought values("+data.get(i).getSellPrice()+",'"+data.get(i).getBarcode()+"','"+data.get(i).getName()+"',"+data.get(i).getBuyPrice()+")";
            	statement.executeUpdate(q);
            	q="insert into buying_bill values('"+data.get(i).getBarcode()+"',TO_DATE('"+LocalDate.now()+"','YYYY-MM-DD'),"+data.get(i).getQuant()+",'"+distName.getText()+"')";
            	statement.executeUpdate(q);
        		...
        		statement.executeUpdate(q);}
            		...
            		q="insert into stored values ("+data.get(i).getSellPrice()+",'"+data.get(i).getName()+"','"+data.get(i).getName()+"',"+data.get(i).getBuyPrice()+","+data.get(i).getQuant()+",TO_DATE('"+data.get(i).getExpDate()+"','YYYY-MM-DD'))";	
            		statement.executeUpdate(q);
                ...
                q="select * from stored where name ='"+data.get(i).getName()+"'";
                    ...
                    if(countb!=0) {
                    	...
                    	q="update stored set exp_date=TO_DATE('" + data.get(i).getExpDate()+"','YYYY-MM-DD') where name = '"+data.get(i).getName()+"'";
                		statement.executeUpdate(q);
                		q="insert into buying_bill values('"+bar+"',TO_DATE('"+LocalDate.now()+"','YYYY-MM-DD'),"+data.get(i).getQuant()+",'"+distName.getText()+"')";
                    	statement.executeUpdate(q);
                    }
                    else {
                    	q="insert into stored values ("+data.get(i).getSellPrice()+",'"+bar+"','"+data.get(i).getName()+"',"+data.get(i).getBuyPrice()+","+data.get(i).getQuant()+",TO_DATE('"+data.get(i).getExpDate()+"','YYYY-MM-DD'))";	
            	...
            	int count4=0;	
            	while(rs3.next()) {count4++;}
            		if(count4==0) {
            			q="insert into bought values("+data.get(i).getSellPrice()+",'"+bar+"','"+data.get(i).getName()+"',"+data.get(i).getBuyPrice()+")";
                    	statement.executeUpdate(q);
            		}
    ...
    void barcodeHandler(KeyEvent event) throws SQLException {
        ...
        if (event.getSource().toString().contains("barcodeReader"))
            if (event.getCode().equals(KeyCode.ENTER)) {
                ...
                /*
                // اذا الباركود هاد موجود بالداتا بيس معناته معلوماته معنا بس بنطلب الكمية
                // وتاريخ الانتهاء بدون اسعار
                if (true)
                    ;
                // اذا لأ بنشوف الاسم اذا موجود بالداتا بيس ، بنعطي الباركود تبع هاد الاسم
                // للمتغير باركود وبناخد كمية وتاريخ انتهاء
                else if (true)
                    ;
                // بالحالتين هدول السلعة موجودة بالمخزن ف ما بنضيفها بس بنعدل ع كميتها وتاريخها
 
                // اذا الباركود والاسم جداد ، بناخد كل المعلومات وبنضيفها للمخزن كسلعة جديدة
                else
                    ;
...
//             
                ...
                dialog.setTitle("الكمية"); dialog.setHeaderText("الرجاء ادخل الكمية");
                quant=Integer.parseInt(dialog.showAndWait().get());
//             
                dialog=new TextInputDialog();
                dialog.setTitle("سعر شراء الوحدة"); dialog.setHeaderText("الرجاء ادخال سعر شراء الوحدة");
                buyPrice=Double.parseDouble(dialog.showAndWait().get());
//             
                dialog=new TextInputDialog();
                dialog.setTitle("سعر بيع الوحدة"); dialog.setHeaderText("الرجاء ادخال سعر بيع الوحدة");
